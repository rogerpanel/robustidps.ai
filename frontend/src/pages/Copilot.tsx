...
const PAGE_CONTEXTS = [
  ...
  // ── UAV / Aerial Defense (robustuavs sub-brand) ─────────────────────
  { id: 'uav_overview',    label: 'UAV Monitor',          query: 'Get the UAV / Aerial Defense monitor overview. What is the framework\'s MCR-vs-J/S curve on UAV-EW-Bench-2026, its DO-326A operational-floor crossing dB, and the current live Lipschitz-Gronwall + RS + PAC-Bayes certificate values? Compare against the three published baselines.', icon: 'plane' },
  { id: 'uav_ew_bench',    label: 'UAV EW-Bench',         query: 'Get the UAV-EW-Bench-2026 measured curves. What is the framework\'s J/S advantage over the strongest baseline (Seq2Seq Transformer)? At what J/S does each configuration cross the DO-326A 0.90 MCR floor? Report the 108,000-flight aggregate.', icon: 'target' },
  { id: 'uav_swarm',       label: 'Swarm Graph',          query: 'Get the swarm graph snapshot. Describe the four time-slice topologies (t1 clean, t2 jamming, t3 intruder, t4 combined view). How does M1 CT-TGNN integrate dh_v/dt across the trajectory, and how does M7 FedGTD re-weight its Stackelberg policy at each topology change?', icon: 'network' },
  { id: 'uav_gnss',        label: 'GNSS Spoof',           query: 'Get the GNSS spoof monitor snapshot. Which satellites are flagged (spoof_confidence above threshold)? What is the current receiver front-end, jamming J/S dB, effective J/S after receiver rejection, fleet disagreement score, and mode? Is the M6 UC-HGP fallback active?', icon: 'radar' },
  { id: 'uav_certificates',label: 'UAV Certificates',     query: 'Get the UAV certification-dashboard live certificates. Report Lipschitz L_g, Gronwall radius, RS certified radius at σ=0.25, PAC-Bayes bound, and the (ε,δ)-DP budget. Interpret each against DO-326A, EU AI Act Art. 15, NIST AI RMF, GOST R 59276-2020, and RF Decree №1701.', icon: 'shield-check' },
  { id: 'uav_perception',  label: 'UAV Perception',       query: 'Get the perception-tester results. What was the last attack run (FGSM/PGD/CW/DeepFool/HopSkipJump/Boundary/Gaussian/FeatureMask/LabelFlip), the epsilon, clean vs adversarial prediction, L2/Linf distortion, and whether the model was fooled? Summarise adversarial robustness at the current sample.', icon: 'eye' },
  { id: 'uav_mission_plan',label: 'Mission Plan Review',  query: 'Get the mission-plan review results. Verdict (approve/block), which findings were flagged (MP-G01 geofence, MP-G02 RTL, MP-G03 altitude, MP-R01 Decree 1701 ack), and severity distribution. Route the audit through the CyberSecLLM surrogate for this call.', icon: 'clipboard-check' },
  { id: 'uav_fleet',       label: 'UAV Fleet Demo',       query: 'Get the live fleet-demo snapshot. Report session_id, ambient J/S dB, fleet MCR, number of UAVs in flight vs completed vs failed, per-UAV attack + defense config, and any autopilot mode flips (nominal → gnss_degraded → rtl).', icon: 'plane' },
] as const
