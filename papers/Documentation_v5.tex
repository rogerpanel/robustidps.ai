% === RobustIDPS.ai Technical Documentation v5 ============================
%  Successor to v4 (June 2026). Reflects the v5.0 release that adds:
%    - The Agent Studio + Agent Security commercial vertical.
%      15 Quickstart agent templates (defenders / attackers /
%      productivity / role-targeted / blank) plus a 4-stage
%      Build → Configure → Test → Ship wizard with Express (no-
%      code) and JSON (engineer) modes.
%    - Six identity modes with a per-page AccessBanner:
%      admin (platform JWT or env bypass), authenticated user,
%      Agent Studio API key, anonymous demo, locked.
%    - Per-user Workspaces: server-side saved BuildWizard state
%      keyed by identity (survives log-out / device change), with
%      save / export / import / archive / delete + admin override.
%    - Deployments registry: track where customer agents run,
%      cross-referenced with the live runtime monitor.
%    - Postgres-ready DB layer + true row-level security policies
%      (RLS_POSTGRES.sql) + JSON→DB migration on boot.
%    - Copilot one-turn orchestrator: eval → red-team → supply-
%      chain → workspace → deployment in a single tool call.
%    - 17 platform IDS models exposed as "special tools" the
%      agent can call, with per-archetype recommendations.
%    - 34-check scanner (added 5 memory-store + 5 credential-
%      vault auditors on top of the v4 12 + 10 + 2 hygiene).
%
%  ── v4 highlights (carried forward) ──
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small\textsc{RobustIDPS.ai --- Technical Documentation v5}}
...
\section{Agent Studio + Agent Security (new in v5)}
\label{sec:agent_studio}

Version 5 promotes RobustIDPS from a research-grade IDS platform to a
dual-vertical product: the existing \emph{network defence} plane
(chapters 3--5 of the dissertation and every prior section of this
document) plus a \emph{commercial agentic-AI} plane. The agentic plane
lets customers build, secure, and ship LLM agents that are guarded by
the same MambaGuard runtime + certified detectors already deployed on
the network side.

\subsection{Two-plane bridge}

The two planes share the same 17-model detection registry
(\S\ref{sec:models}), the same PostgreSQL persistence layer, the same
SOC Copilot with tool-use, and the same slowapi rate limiter. What
they add is a per-tenant identity fabric and a build-time surface that
compiles to the same runtime the network plane already runs. Every
agent a customer ships is an ordinary MambaGuardClient consumer of
\path{/api/agent-studio/runtime/ingest}; the runtime dashboard
(\path{/agent-studio/runtime}) is the same one that visualises the
network plane's fleet.

\subsection{Quickstart template catalog}

15 archetypes ship out of the box. The role picker on
\path{/agent-studio/quickstart} filters the catalog per persona:
...
\midrule
A (defender)  & \texttt{soc\_triage}          & Tier-1 SIEM alert triage \\
A (defender)  & \texttt{incident\_commander}  & Incident war-room orchestration \\
A (defender)  & \texttt{compliance\_auditor}  & ISO 42001 / EU AI Act policy Q\&A \\
A (defender)  & \texttt{vuln\_triage}         & CVE $\to$ SBOM $\to$ owner triage \\
A (defender)  & \texttt{mcp\_auditor}         & Continuous 24-check MCP scanner \\
A (defender)  & \texttt{threat\_hunter}       & Hypothesis-driven log-lake hunt \\
B (attacker)  & \texttt{red\_team\_operator}  & Garak-suite driver + report \\
B (attacker)  & \texttt{pentest\_recon}       & Authorised recon within scope envelope \\
B (attacker)  & \texttt{phishing\_trainer}    & Authorised awareness campaigns \\
C (product.)  & \texttt{customer\_support}    & RAG over docs, scope-locked \\
C (product.)  & \texttt{billing\_copilot}     & Strict-scope invoice Q\&A, refund-gated \\
C (product.)  & \texttt{docs\_qa}             & Internal KB Q\&A, hallucination-gated \\
C (role)      & \texttt{network\_traffic\_monitor} & Flow anomaly triage $\to$ ticket \\
C (role)      & \texttt{uav\_swarm\_coordinator} & Satellite-linked swarm coord + safe RTB \\
Blank         & \texttt{blank}                & Starter with core toolset only \\
\bottomrule
\end{tabular}
\caption{15 Quickstart archetypes shipped in v5, with tier
(A=defender, B=attacker, C=productivity or role-targeted).}
\end{table}

Each template carries \texttt{environment} (runtime, network policy,
packages, MCP servers, env vars, secrets), \texttt{test\_inputs[]}
(canonical probes), and \texttt{integration\_snippets[]} (Python for
the recommended framework, cURL, and a Kubernetes manifest stub).

\subsection{Four-stage BuildWizard}

\begin{enumerate}
\item \textbf{Create agent} --- Express or JSON. Express is a
      five-question no-code form (name, purpose, allowed tools, network
      policy, memory, on-critical behaviour) that compiles to the same
      spec engineers see in JSON mode. Users flip between modes; the
      form re-seeds itself from any JSON edits.
\item \textbf{Configure environment} --- runtime, network policy
      (\texttt{outbound\_open} / \texttt{outbound\_blocked} /
      \texttt{allowlist}), packages, MCP servers with per-server policy
      (\texttt{read\_only} / \texttt{write\_audited}), env vars,
      secrets (shown in red as an escalation cue).
\item \textbf{Start session} --- sandboxed test conversation. Each
      turn ships input + output through the 34-check Aegis scanner;
      verdict bubbles show \texttt{allow / warn / block} with per-turn
      finding codes and the LLM provider chip (Anthropic / OpenAI /
      Google / DeepSeek, or synthetic fallback when no key set).
\item \textbf{Integrate} --- Python / cURL / Kubernetes snippets, plus
      a green \textbf{Register this deployment} button that prefills
      the Deployments form.
\end{enumerate}

A right-column \textbf{Side Suggestions} card follows the user through
all four steps. It reads
\path{/api/agent-studio/build-suggestions/\{template\}?step=N} and
renders (i) 4--6 tips per (step, archetype), and (ii) a
\textbf{Related pages} section with deep-links to other verticals ---
e.g. the UAV Swarm Coordinator template links to
\path{/uav/mission-plan}, \path{/uav/swarm}, \path{/uav/fleet-demo},
and \path{/uav/certification}; the Network Traffic Monitor links to
\path{/live-monitor}, \path{/analytics}, and \path{/threat-response}.
Turns the templates into real bridges between planes rather than
prose references.

\subsection{Platform models as special tools}

Every one of the 17 detection / response / certified-defence models
in the shared registry (\S\ref{sec:models}) is attachable to any
agent as a "special tool". The BuildWizard's Platform Model picker
shows the catalog grouped by category (\texttt{ensemble},
\texttt{temporal}, \texttt{federated}, \texttt{foundation},
\texttt{clrl}, \texttt{certified}, \texttt{self\_supervised},
\texttt{pqc}, \texttt{llm\_protocol}) with per-archetype recommended
defaults --- e.g. \texttt{network\_traffic\_monitor} recommends
\texttt{surrogate + neural\_ode + ssl\_graph\_anomaly +
multi\_agent\_pqc + sode\_guard + lipmamba}. Selected models fold
into \texttt{spec.platform\_models[]} as \texttt{[\{id, role\}]}
entries; the runtime SDK dispatches each as a tool the agent can
call.

\subsection{Six-way identity fabric}

Every Agent Studio page renders an \textbf{AccessBanner} showing the
active mode:
...
Mode & Trigger & Access \\
\midrule
Admin (platform JWT)  & user.role == admin  & Unrestricted \\
Admin (env bypass)    & \texttt{AGENT\_STUDIO\_AUTH\_DISABLED=1} & Unrestricted \\
Authenticated user    & platform JWT, non-admin & Tier-scoped per \texttt{user.subscription\_tier} \\
API key holder        & \texttt{Bearer rids\_live\_\ldots} & Tier-scoped per issued key \\
Anonymous demo        & \texttt{AGENT\_STUDIO\_DEMO\_MODE=1} & Community tier, cannot persist \\
Locked                & none of the above   & 401 with actionable hint \\
\bottomrule
\end{tabular}
\caption{Six identity modes, resolved in the auth dependency in
priority order.}
\end{table}

The frontend always prefers the platform JWT over the API key when
both are stored, so a logged-in admin gets full access on every
Agent Studio page without re-pasting a key.

\subsection{Per-user Workspaces}

A user's in-progress build (wizard step + spec + environment +
selected platform models + chat history + snippet tab) persists in
the \texttt{agent\_studio\_workspaces} table keyed by
\texttt{owner\_id}. Survives log-out and device change; strict
per-user isolation via the \texttt{scoped()} helper and (on Postgres)
a row-level security policy in \path{RLS_POSTGRES.sql}. Admin sees
all users' workspaces via the same endpoints with the admin role
attached to the customer dict.

Four lifecycle operations: \textbf{Save} (upsert),
\textbf{Save as}, \textbf{Load}, \textbf{Export} (JSON download),
\textbf{Import} (upload re-owns to the importing user, no
privilege-escalation risk), \textbf{Archive} (soft),
\textbf{Delete} (hard). Demo-mode visitors are refused on save and
import with a clear hint to sign in.

\subsection{Deployments registry}

\path{/agent-studio/deployments} is the "where is my agent running?"
view. Each row cross-references a \texttt{runtime\_agent\_id} used
by the customer's MambaGuardClient; status is derived from the live
runtime monitor: \emph{healthy} (block\_rate $<$ 5\%), \emph{degraded}
($\ge$ 5\% or warn-tier surface), \emph{stale} (no recent events),
\emph{retired} (manual).

The BuildWizard's Step 4 "Register this deployment" CTA prefills the
form (template\_id, suggested runtime\_agent\_id, sensible cloud
default per snippet type). One Submit and telemetry flows into the
dashboard the same way the network plane's fleet does.

\subsection{One-turn orchestrator}

\begin{lstlisting}[language=bash]
curl -sf -X POST https://robustidps.ai/api/agent-studio/orchestrate \
  -H "Authorization: Bearer $ROBUSTIDPS_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{"template_id":"network_traffic_monitor",
        "cloud":"docker_self","tier":"production",
        "region":"eu-central-1"}'
\end{lstlisting}

Chains in one turn: 5-eval harness $\to$ 12-probe Garak red-team $\to$
supply-chain scan (default \texttt{cybersec\_llm}) $\to$ workspace
save (idempotent by name) $\to$ deployment register. Returns
\texttt{run\_ids + workspace\_id + deployment\_id + next\_steps} (deep
URLs to dossier / runtime / workspaces / deployments). The SOC Copilot
exposes it as \texttt{orchestrate\_build\_and\_ship\_agent}; a customer
can literally say "build me a soc\_triage agent" and get a fully
shipped, verifiable artefact back.

\subsection{Scanner check families (34 total in v5)}
...
\# & Family \\
\midrule
12 & MCP framing (MCP-S01..12) --- unchanged from v4 \\
10 & OWASP LLM Top-10 (LLM-01..10) --- unchanged from v4 \\
 5 & Memory-store auditor (MEM-01..05, ASI04) --- \emph{new in v5} \\
 5 & Credential-vault auditor (VLT-01..05, ASI06) --- \emph{new in v5} \\
 2 & Cross-cutting hygiene (HYG-01..02) --- unchanged \\
\bottomrule
\end{tabular}
\end{center}

The two new auditor families target OWASP-Agentic ASI04 (memory
poisoning: unvalidated writes, untagged provenance, cross-tenant
memory, no TTL, no trust-tier filtering) and ASI06 (excessive agency:
inlined secrets, unbound vault refs, wildcard policies, no-TTL
leases, secrets echoed in output).

\subsection{Postgres row-level security}

The Agent Studio DB layer works identically on SQLite (dev) and
Postgres (production). Application-layer isolation via
\texttt{scoped(query, model, customer, customer\_id\_col)} works
everywhere; Postgres-true RLS is a separate opt-in
(\path{backend/plugins/agent_studio/RLS_POSTGRES.sql}) that adds one
...
Same shape covers \texttt{agent\_studio\_customers},
\texttt{agent\_studio\_api\_keys}, \texttt{agent\_studio\_admin\_grants},
\texttt{agent\_studio\_deployments}, \texttt{agent\_studio\_sessions}
$+$ messages, and the three history tables (eval / red-team /
supply-chain).

\subsection{Reference architecture snapshot}

\begin{center}
\begin{tikzpicture}[font=\small,node distance=6mm]
\node[draw,rounded corners,fill=robustblue!12,minimum width=3.7cm,minimum height=1.0cm,align=center]
  (portal) {\textbf{Agent Studio Portal}\\3-column hub view};
\node[draw,rounded corners,fill=socgreen!15,minimum width=3.7cm,minimum height=1.0cm,align=center,below=of portal]
  (build) {\textbf{BuildWizard}\\Express $\leftrightarrow$ JSON (4 steps)};
\node[draw,rounded corners,fill=newamber!15,minimum width=3.7cm,minimum height=1.0cm,align=center,below=of build]
  (orch) {\textbf{Orchestrator}\\one-turn build/secure/ship};
\node[draw,rounded corners,fill=slateteal!20,minimum width=3.7cm,minimum height=1.0cm,align=center,right=1.4cm of portal]
  (ws) {\textbf{Workspaces}\\per-user, exportable};
\node[draw,rounded corners,fill=slateteal!20,minimum width=3.7cm,minimum height=1.0cm,align=center,right=1.4cm of build]
  (dep) {\textbf{Deployments}\\live runtime telemetry};
\node[draw,rounded corners,fill=llmred!12,minimum width=3.7cm,minimum height=1.0cm,align=center,right=1.4cm of orch]
  (dossier) {\textbf{Assurance Dossier}\\auditor-ready PDF};
\draw[-{Latex}] (portal) -- (build);
\draw[-{Latex}] (build)  -- (orch);
\draw[-{Latex}] (build)  -- (ws);
\draw[-{Latex}] (build)  -- (dep);
\draw[-{Latex}] (dep)    -- (dossier);
\draw[-{Latex}] (ws)     -- (dossier);
\draw[-{Latex}] (orch)   -- (dossier);
\end{tikzpicture}
\end{center}

\subsection{CLI + SDK surface}

The \texttt{robustidps} console\_script gains a full \texttt{agent}
subcommand group in v5:

\begin{lstlisting}[language=bash,basicstyle=\ttfamily\scriptsize]
robustidps agent scan FILE [--kind mcp_manifest|system_prompt|...]
robustidps agent eval FILE
robustidps agent red-team FILE [--garak]
robustidps agent supply-chain MODEL_ID [--live]
robustidps agent dossier [--vertical agent_studio] [-o dossier.json]
robustidps agent templates [--tier A|B|C|blank]
robustidps agent template TEMPLATE_ID [--spec-only]
robustidps agent session {start,message,show,list}
robustidps agent key {issue,revoke}
robustidps agent whoami --customer cust_...
robustidps agent activity [--limit 5]
robustidps agent deployments {list,register,retire}
robustidps agent admin {grant,list,revoke,whoami}
\end{lstlisting}

The AegisAgents Kit ships wrappers for 11 frameworks
(\texttt{langgraph, crewai, mcp, a2a, anp, openai\_agents, autogen,
pydantic\_ai, dspy, strands, smolagents}) plus a pytest plugin
(\texttt{--aegis-fail-on-warn}) that fails the build on any block-
tier verdict (or warn-tier if strict).

\subsection{SOC Copilot expansion (v4 $\to$ v5)}

The Copilot registry grew from the 34 tools shipped in v4 to
\textbf{61 tools} in v5 --- 27 new tools cover the Agent Studio
surface: templates, sessions, runtime snapshots, supply-chain scans,
customer / grants / API keys, workspaces (list, stats), deployments
(list, detail, register, retire), platform models
(list, recommend), activity rollup, LLM-provider info, and the
orchestrator. Every tool is symmetric in the TOOLS registry and the
dispatcher --- verified in CI on every push
(\path{.github/workflows/aegis-gate.yml}).


\section{V4 $\to$ V5 Change Log}
\label{appx:changelog}

\subsection*{New features in v5 (Agent Studio + Agent Security)}
\begin{itemize}[leftmargin=1.2em,itemsep=1pt]
\item \textbf{15 Quickstart agent templates} across defender / attacker / productivity / role tiers, each with \texttt{environment} + \texttt{test\_inputs} + \texttt{integration\_snippets} baked in.
\item \textbf{4-stage BuildWizard} (Create $\to$ Configure $\to$ Test $\to$ Ship) with Express (no-code) and JSON (engineer) modes and a persistent workspace toolbar.
\item \textbf{Six identity modes} (admin JWT, env bypass, authenticated user, API key, anonymous demo, locked) with a per-page AccessBanner surfacing the active one.
\item \textbf{Per-user Workspaces} with Save / Save-as / Load / Export / Import / Archive / Delete + admin override. Strict per-user isolation at both app and DB (Postgres RLS) layers.
\item \textbf{Deployments registry} cross-referenced with the live runtime monitor. Colour-coded status (healthy / degraded / stale / retired) per instance.
\item \textbf{Copilot one-turn orchestrator} (\texttt{orchestrate\_build\_and\_ship\_agent}): eval $\to$ red-team $\to$ supply-chain $\to$ workspace $\to$ deployment in one call.
\item \textbf{17 platform models as special tools}, with per-archetype recommendations surfaced in the wizard.
\item \textbf{10 new scanner checks} (MEM-01..05 for ASI04 memory poisoning, VLT-01..05 for ASI06 vault leaks); total 34 checks.
\item \textbf{Cross-vertical bridges} from templates to \path{/uav/*}, \path{/live-monitor}, \path{/analytics}, \path{/threat-response}, \path{/mcp-security}, \path{/red-team-arena}, \path{/cve-mapper}, \path{/compliance}, \path{/atlas}.
\item \textbf{Rate-limited endpoints} (slowapi) with per-route quotas tunable via env; 7 quota classes.
\item \textbf{Real-LLM dispatch in test sessions} (4-provider: Anthropic / OpenAI / Google Gemini / DeepSeek) with deterministic synthetic fallback for air-gapped deploys.
\item \textbf{Postgres-true RLS} (\path{RLS_POSTGRES.sql}, 7 tenant tables + workspaces) as opt-in over the app-layer \texttt{scoped()} helper.
\item \textbf{CI dogfood}: \path{.github/workflows/aegis-gate.yml} runs \texttt{pytest --aegis-fail-on-warn} on every push. Green as of the v5 tag.
\item \textbf{4 starter agent repos} (\path{examples/agents/\{soc_triage,billing_copilot,red_team_operator,docs_qa\}}) $+$ Fly.io / Modal / Vercel deploy recipes under \path{examples/agents/_deploy/}.
\end{itemize}

\subsection*{V4 features (with git commit references)}
\begin{itemize}[nosep]
