...
def _build_predictions(features, metadata, labels_encoded, label_names,
    ...
    for i in range(len(preds)):
        ...
        rows.append({
            ...
            "src_port": int(metadata["src_port"].iloc[i]) if "src_port" in metadata.columns else (int(metadata["Src Port"].iloc[i]) if "Src Port" in metadata.columns else 0),
            "dst_port": int(metadata["dst_port"].iloc[i]) if "dst_port" in metadata.columns else (int(metadata["Dst Port"].iloc[i]) if "Dst Port" in metadata.columns else 0),
            "protocol": str(metadata["protocol"].iloc[i]) if "protocol" in metadata.columns else (str(metadata["Protocol"].iloc[i]) if "Protocol" in metadata.columns else ""),
