...
def _build_predictions(features, metadata, labels_encoded, label_names,
    ...
    for i in range(len(preds)):
        ...
        rows.append({
            ...
            "src_port": int(metadata[next((c for c in ["src_port", "Src Port", "sport", "srcport", "Sport", "source_port"] if c in metadata.columns), "")].iloc[i]) if any(c in metadata.columns for c in ["src_port", "Src Port", "sport", "srcport", "Sport", "source_port"]) else 0,
            "dst_port": int(metadata[next((c for c in ["dst_port", "Dst Port", "dsport", "dstport", "Dport", "dport", "destination_port", "Destination Port"] if c in metadata.columns), "")].iloc[i]) if any(c in metadata.columns for c in ["dst_port", "Dst Port", "dsport", "dstport", "Dport", "dport", "destination_port", "Destination Port"]) else 0,
            "protocol": str(metadata[next((c for c in ["protocol", "Protocol", "proto", "Proto"] if c in metadata.columns), "")].iloc[i]) if any(c in metadata.columns for c in ["protocol", "Protocol", "proto", "Proto"]) else "",
